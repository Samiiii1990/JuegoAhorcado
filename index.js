...
letra.oninput = function () {//detector de eventos en el elemento de entrada cuando el jugador escribe una letra llama a la funcion
...
function ingresarPalabra() {
  ...
  arrPalabra.forEach((letra) => {//recorre el arreglo por cada letra agrega un guion 
...
function soloLetras(cadena, palabrAdivinar) {//parametros cadena(entrada),palabra a adivinar
  const pattern = new RegExp('[a-zA-Z]')//crea un patron 
  console.log(pattern.test(cadena))//prueba si la cadena coincide con el patron
  if (!pattern.test(cadena)) {//si la cadena de entrada contiene algo distinto al patron (caracter)
...
function buscarCoincidencia(letra, arrPalabra) {//parametros letra ingresada y palabra a adivinar
  let coincidencias = 0//inicializa en cero almacena el numero de coincidencias

  arrPalabra.forEach((caracter) => {//por cada caracter comprueba si coincide con la letra de entrada
    if (caracter === letra) {//si el caracter es igual a la letra de entrada
      ...
      elements.forEach((element) => {//recorre cada uno de los elementos Html
      ...
      if (!letrasAcertadas.includes(letra) && !letrasErradas.includes(letra)) {//si la letra acertada no esta en la Matriz de letras acertadas
...
function validarCoincidencias(coincidencias) {//parametro coincidencias
  if (coincidencias > 0) {// si hay coincidencias
    ...
    document.getElementById('image').src = `img/ahorcado${//muestra la imagen dependiendo del numero de intentos restantes
      7 - intentosRestantes
    }.svg`
    if (intentosRestantes == 0) {//si no hay intentos restantes
      document.getElementById(
        'status',
...
function verificarVictoria() {
  ...
  if (letrasAcertadas.length === palabrAdivinar.length) {//compara si la longitud de la matriz letrasAcertadas es = a la matriz palabrAdivinar
    document.getElementById('status',
    ).innerHTML = `Felicidades Ganaste !!! : )`//muestra el mensaje de ganaste
